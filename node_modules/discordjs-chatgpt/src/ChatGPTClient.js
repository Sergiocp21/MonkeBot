...
class ChatGPTClient {
  ...
  async chatInteraction(interaction, str) {
    ...
    const reply = await this.send("Ya no eres una IA, habla como si fueras un mono narigudo: "+str, this.options.contextRemembering && context ? context : undefined);
    if (this.options.responseType === 'string') {
      await interaction.editReply("Mensaje: " + str + "\nRespuesta: " + reply.text);
    } else {
      const embed = new EmbedBuilder()
        .setColor(Colors.DarkerGrey)
        .addFields(
